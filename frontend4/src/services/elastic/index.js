...
export function search_simple( sender, jsonQuery)
{
    ...
    if(!token) {
        ...
        axios.post(BUSINESS_SERVER_URL+'/elastic/search/simple/', prepare_q1( jsonQuery), config)
            .then(({data}) => {
...
export function search_drill( sender, jsonQuery)
{
    ...
    if(!token) {
        ...
        console.log(config);
        console.log(prepare_q1( jsonQuery));
        axios.post(BUSINESS_SERVER_URL+'/elastic/search/drill/', prepare_q1( jsonQuery), config)
            .then(({data}) => {
                sender.setState({result: data, loading:false});
            })
            .catch( ( err ) => {
